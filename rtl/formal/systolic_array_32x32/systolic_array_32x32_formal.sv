module sovryn_pan_stem_systolic_array_32x32_formal;
  (* anyseq *) reg clk;
  (* anyseq *) reg rst_n;
  (* anyseq *) reg load_valid;
  (* anyseq *) reg load_matrix_sel;
  (* anyseq *) reg [9:0] load_addr;
  (* anyseq *) reg [7:0] load_data;
  (* anyseq *) reg compute_valid;
  (* anyseq *) reg read_valid;
  (* anyseq *) reg [9:0] read_addr;

  wire out_valid;
  wire [23:0] read_data;

  sovryn_pan_stem_systolic_array_32x32 dut (
    .clk(clk),
    .rst_n(rst_n),
  ...
  always @(posedge clk) begin
    ...
    if (f_past_valid && rst_n && $past(rst_n && read_valid && !load_valid && !compute_valid)) begin
      assert(out_valid);
      assert(read_data == 24'd0);
    end
  end
