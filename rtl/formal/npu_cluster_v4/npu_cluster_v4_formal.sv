module sovryn_pan_stem_npu_cluster_v4_formal;
  (* anyseq *) reg clk;
  (* anyseq *) reg rst_n;
  ...
  sovryn_pan_stem_npu_cluster_v4_top dut (
    .clk(clk),
    .rst_n(rst_n),
  ...
  always @(posedge clk) begin
    ...
    if (rst_n) begin
      assume(!(dispatch_valid && read_valid));
    end
    ...
    if (f_past_valid && $past(rst_n && dispatch_valid)) begin
      assert(out_valid);
