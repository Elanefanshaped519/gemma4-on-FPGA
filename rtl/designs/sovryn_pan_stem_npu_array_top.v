...
`include "sovryn_pan_stem_npu_array_tile_queue.v"
`include "sovryn_pan_stem_npu_array_tile_datapath.v"
`include "sovryn_pan_stem_npu_array_tile_commit.v"
`include "sovryn_pan_stem_npu_array_tile.v"
`include "sovryn_pan_stem_npu_array_observe_readback.v"

module sovryn_pan_stem_npu_array_top(
  input wire clk,
  input wire rst_n,
  ...
  generate
    for (row_index = 0; row_index < MAX_ROWS; row_index = row_index + 1) begin : row_gen
      for (col_index = 0; col_index < MAX_COLS; col_index = col_index + 1) begin : col_gen
        ...
        localparam integer WEST_INDEX = (row_index * MAX_COLS) + (col_index - 1);
        ...
        sovryn_pan_stem_npu_array_tile #(
          .LOCAL_ROW(row_index),
          .LOCAL_COL(col_index),
